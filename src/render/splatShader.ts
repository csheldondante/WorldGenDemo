...
const SNOISE = /* glsl */ `
  ...
  vec3 permuteX(vec3 x){return mod289_3(((x*34.0)+1.0)*x);}
  float snoise(vec2 v){
    ...
    x12.xy -= i1;
    i = mod289_2(i);
    vec3 p = permuteX(permuteX(i.y + vec3(0.0, i1.y, 1.0))
...
export function makeSplatMaterial(inputs: SplatInputs): THREE.MeshStandardMaterial {
  const mat = new THREE.MeshStandardMaterial({ roughness: 0.95, metalness: 0.0, color: 0xffffff });
  ...
  mat.onBeforeCompile = (shader) => {
    ...
    // ---- vertex: emit a custom varying with the original UV (independent of USE_MAP)
    ...
    // ---- fragment: declare uniforms and the varying, then patch albedo
    ...
    // <color_fragment> runs unconditionally for MeshStandardMaterial; perfect injection point.
    shader.fragmentShader = shader.fragmentShader.replace(
      "#include <color_fragment>",
      `#include <color_fragment>
        vec2 mUv = vTerrainUv;
        vec4 dist = texture2D(uDistanceField, mUv);
        vec4 w = exp(-dist * uSharpness);
        ...
        vec2 nrm = texture2D(uBoundaryField, mUv).rg * 2.0 - 1.0;
        vec2 perp = vec2(-nrm.y, nrm.x);
        float n = snoise(mUv * uNoiseScale);
        vec2 warpedUv = mUv * uTile + perp * n * uWarpAmt * uTile;
  ...
  mat.customProgramCacheKey = () => "splat-jfa-v2";
  return mat;
}
