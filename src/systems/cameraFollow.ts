...
/**
 * Third-person follow camera. Reads InputMapBuffer.lookDelta for free-look
 * (mouse + right stick already blended in radians/tick by InputMapperSystem),
 * reads the player's TransformBuffer for the focus point, writes CameraBuffer
 * pos/yaw/pitch. When there's no character it leaves the camera as-is so
 * Loading state still shows something.
 */
export function createCameraFollowSystem(): SystemDescriptor {
  return {
    id: CAMERA_FOLLOW_SYSTEM_ID,
    description:
      "Third-person follow camera. Reads InputMapBuffer.lookDelta (device-agnostic), TransformBuffer (player), writes CameraBuffer pos/yaw/pitch behind+above the player.",
    buffers: [
    ...
    runsAfter: [STATE_MACHINE_SYSTEM_ID, SURFACE_CONSTRAINT_SYSTEM_ID, INPUT_MAPPER_SYSTEM_ID, CHARACTER_INPUT_SYSTEM_ID],
    execute: ({ buffer }) => {
