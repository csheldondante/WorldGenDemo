...
export function createTangentInputMapperSystem(): SystemDescriptor {
  return {
    ...
    runsAfter: [
      ...
      // Read profile after BindingSwapSystem has installed the active
      // binding's CharacterControllerProfile this tick, so binding
      // swaps take effect on the same tick they're emitted.
      "bindingSwapSystem",
    ],
    execute: ({ buffer }) => {
